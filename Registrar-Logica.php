...
function encrypt($string, $key) {
   ...
   return base64_encode($result);
	...
	{
		header("Location: Registrar-Vista.php?errordatos=si");
	}
	$contra =  $_POST["Contra"];
	if(strlen($contra) < 8)
	{
		header("Location: Registrar-Vista.php?errorcontrasena=si");
	}
	...
	while( $row = mysql_fetch_array ( $result )) 
	{
		if($row["Ci"] == $_POST["Ci"])
		{
			header("Location: Registrar-Vista.php?errorusuario=si");
		}
	}
	mysql_close($bd);
